(game-module "awls-testbed"
  (title "A World Lost Slowly")
  (version "1.17")

;; Currently the specops use the detonate-with-attack (but they cause no damage)
;; Most units use the treasury-gain/loss functions (for a new 'prestige' material)
;; Combat units use cxp-per-destroy

  (blurb "Testbed for fancy stuff.")
  (variants
     (sequential true)
     (world-seen true)
     (see-all false)
...
;;;  (possible-sides "None") (namer "None")
  (point-value 18) (detonate-with-attack 100)
  (ground true)
...
(material-type prestige (name "Prestige"))
...
(table treasury-gain-per-destroy
	(u* prestige 1)
)

(table treasury-loss-per-destroyed
	(u* prestige 1)
)

(table treasury-gain-per-capture
	(u* prestige 1)
)

(table treasury-loss-per-captured
	(u* prestige 1)
)
